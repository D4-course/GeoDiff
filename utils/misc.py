...
def get_logger(name, log_dir=None, log_fn='log.txt'):
    'Log information on training or testing'
...
def seed_all(seed):
    'Setup up the random seeds for all modules to get repeatable results'
...
def inf_iterator(iterable):
    'Placeholder class for iterables'
...
def log_hyperparams(writer, args):
    'Log hyperparameters of the model from tensorboard'
...
def int_tuple(argstr):
    'Convert args to tuple of ints'
    return tuple(map(int, argstr.split(',')))


def str_tuple(argstr):
    'Convert args to tuple of strs'
...
def get_checkpoint_path(folder, it=None):
    'Get the path to the checkpoint created'
    if it is not None:

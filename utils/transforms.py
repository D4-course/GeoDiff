import copy
import torch
from torch_geometric.data import Data  # pylint: disable=import-error
...
class AddHigherOrderEdges():
    ...
    def binarize(self, inputs):
        return torch.where(inputs > 0, torch.ones_like(inputs), torch.zeros_like(inputs))
    ...
    def get_higher_order_adj_matrix(self, adj, order):
        ...
        adj_mats = [torch.eye(adj.size(0), dtype=torch.long, device=adj.device), \
                    self.binarize(adj + torch.eye(adj.size(0), dtype=torch.long, \
                    device=adj.device))]
    ...
    def __call__(self, data: Data):
        ...
        data.edge_index, data.edge_type = coalesce(new_edge_index, \
                                                   new_edge_type.long(),\
                                                   Num_Nodes, Num_Nodes) # modify data
...
class AddEdgeLength():
    'Normalise the edge length or get the current length'
    def __call__(self, data: Data):
        ...
        data.edge_length = d
        return data
    def __get_length__(self, data: Data):
        pos = data.pos
        row, col = data.edge_index
...
class AddAngleDihedral():
    'Create a list of angles from the adjacency matrix'
    def __init__(self):
        super().__init__()
    ...
    def __call__(self, data: Data):
        if 'is_bond' in data:
            bond_mat = to_dense_adj(data.edge_index, \
                                    edge_attr=data.is_bond).long().squeeze(0) > 0
        else:

"""
A set of common functions to be used for the models
"""
...
from torch_geometric.nn import radius_graph, radius # pylint: disable=import-error
from torch_geometric.utils import to_dense_adj, dense_to_sparse # pylint: disable=import-error
from torch_scatter import scatter_mean, scatter_add, scatter_max # pylint: disable=import-error
from torch_sparse import coalesce # pylint: disable=import-error
...
class MeanReadout(nn.Module):
    ...
    def forward(self, data, inputs):
        ...
        """
        output = scatter_mean(inputs, data.batch, dim=0,
...
class SumReadout(nn.Module):
    ...
    def forward(self, data, inputs):
        ...
        Returns:
            Tensor: graph representations
        """
        output = scatter_add(inputs, data.batch, dim=0,
                             dim_size=data.num_graphs)
        return output
...
class MultiLayerPerceptron(nn.Module):
    ...
    def forward(self, inputs):
        ...
        for i, layer in enumerate(self.layers):
            fwd_inp = layer(fwd_inp)
            if i < len(self.layers) - 1:
                if self.activation:
                    fwd_inp = self.activation(fwd_inp)
                if self.dropout:
                    fwd_inp = self.dropout(fwd_inp)
        return fwd_inp


def assemble_atom_pair_feature(node_attr, edge_index, edge_attr):
    "Pair node attributes and edge features"
...
def generate_symmetric_edge_noise(num_nodes_per_graph, edge_index, edge2graph, device):
    "Generate noise for each edge in the graph"
    ...
    undirected_edge_id = node_large * \
        (node_large + 1) + node_small + edge_start
...
def _extend_graph_order(num_nodes, edge_index, edge_type, order=3):
    ...
    def binarize(inps):
        return torch.where(inps > 0, torch.ones_like(inps), torch.zeros_like(inps))
    ...
    type_mat = to_dense_adj(
        edge_index, edge_attr=edge_type).squeeze(0)   # (N, N)
    type_highorder = torch.where(adj_order > 1, num_types +
                                 adj_order - 1, torch.zeros_like(adj_order))
    assert (type_mat * type_highorder == 0).all()
    type_new = type_mat + type_highorder

    new_edge_index, new_edge_type = dense_to_sparse(type_new)
    _, _ = dense_to_sparse(adj_order)

    # data.bond_edge_index = data.edge_index  # Save original edges
    new_edge_index, new_edge_type = coalesce(new_edge_index,
                                             new_edge_type.long(),
                                             num_atoms, num_atoms)  # modify data
    ...
    #  edge_order.long(), N, N) # modify data
    # assert (data.edge_index == edge_index_1).all()

    return new_edge_index, new_edge_type


def _extend_to_radius_graph(pos, edge_index, edge_type, cutoff,\
                            batch, unspecified_type_number=0, is_sidechain=None):

    assert edge_type.dim() == 1
    num_coords = pos.size(0)

    bgraph_adj = torch.sparse.LongTensor(
        edge_index,
        edge_type,
        torch.Size([num_coords, num_coords])
    )

    if is_sidechain is None:
        rgraph_edge_index = radius_graph(
            pos, r=cutoff, batch=batch)    # (2, E_r)
    else:
        ...
        assign_index = radius(x=pos, y=sidechain_pos,
                              r=cutoff, batch_x=batch, batch_y=sidechain_batch)
        r_edge_index_x = assign_index[1]
        r_edge_index_y = assign_index[0]
        r_edge_index_y = sidechain_index[r_edge_index_y]

        rgraph_edge_index1 = torch.stack(
            (r_edge_index_x, r_edge_index_y))  # (2, E)
        rgraph_edge_index2 = torch.stack(
            (r_edge_index_y, r_edge_index_x))  # (2, E)
        rgraph_edge_index = torch.cat(
            (rgraph_edge_index1, rgraph_edge_index2), dim=-1)  # (2, 2E)
        # delete self loop
    ...
    rgraph_adj = torch.sparse.LongTensor(
        rgraph_edge_index,
        torch.ones(rgraph_edge_index.size(1)).long().to(
            pos.device) * unspecified_type_number,
        torch.Size([num_coords, num_coords])
    )
...
def coarse_grain(pos, node_attr, subgraph_index, batch):
    'Create a graph by considering each subgraph as a single node'
    cluster_pos = scatter_mean(
        pos, index=subgraph_index, dim=0)    # (num_clusters, 3)
    cluster_attr = scatter_add(
        node_attr, index=subgraph_index, dim=0)  # (num_clusters, H)
    cluster_batch, _ = scatter_max(
        batch, index=subgraph_index, dim=0)  # (num_clusters, )

    return cluster_pos, cluster_attr, cluster_batch


def batch_to_natoms(batch):
    'Get number of atoms in each batch'
...
def get_complete_graph(natoms):
    ...
    Returns:
        edge_index: (2, N_1 + N_2 + ... + N_{B-1}), 
        where N_i is the number of nodes of the i-th graph.
    ...
    atom_count_sqr = torch.arange(
        num_atom_pairs, device=num_atom_pairs.device) - index_sqr_offset

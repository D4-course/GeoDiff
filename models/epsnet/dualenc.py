"""
Apply Edge, Schnet and MLP Encoders (in that order) for local and global features
and going over timesteps adding noise to it and denoising it and then computing the
loss over the features and structure generated
"""
...
def get_beta_schedule(beta_schedule, *, beta_start, beta_end, num_diffusion_timesteps):
    """Create a timeline using a specified function"""
    def sigmoid(inp):
...
class DualEncoderEpsNetwork(nn.Module):
    ...
    def __init__(self, config):
        ...
        '''
        self.model_global = nn.ModuleList([self.edge_encoder_global,\
                                           self.encoder_global, self.grad_global_dist_mlp])
        self.model_local = nn.ModuleList([self.edge_encoder_local,\
                                          self.encoder_local, self.grad_local_dist_mlp])
    ...
    def forward(self, atom_type, pos, bond_index, bond_type, batch, time_step,
        ...
        # Encode global features, local features or features of pairs of atoms
        ...
        if isinstance(sigma_edge, torch.Tensor):
            ...
            edge_inv_local = self.grad_local_dist_mlp(h_pair_local) *\
                             (1.0 / sigma_edge) # (E_local, 1)
        # Pass through MLP and generate the results before diffusion
        if return_edges:
    ...
    def get_loss(self, atom_type, pos, bond_index, bond_type, batch, num_nodes_per_graph, num_graphs,
                 ...
                 return_unreduced_edge_loss=False, extend_order=True, extend_radius=True,\
                 is_sidechain=None):
        '''
        Get loss for different different model types
        '''
        if self.model_type == 'diffusion':
            return self.get_loss_diffusion(atom_type, pos, bond_index, bond_type, batch, num_nodes_per_graph,\
                                           num_graphs, anneal_power, return_unreduced_loss,\
                                           return_unreduced_edge_loss, extend_order, extend_radius, is_sidechain)
        return self.get_loss_dsm(atom_type, pos, bond_index,\
                                    bond_type, batch, num_nodes_per_graph, num_graphs,\
                                    anneal_power, return_unreduced_loss, return_unreduced_edge_loss,\
                                    extend_order, extend_radius, is_sidechain)


    def get_loss_diffusion(self, atom_type, pos, bond_index, bond_type, batch, num_nodes_per_graph,\
                           num_graphs, anneal_power=2.0, return_unreduced_loss=False,\
                           return_unreduced_edge_loss=False, extend_order=True,\
                           extend_radius=True, is_sidechain=None):
        '''Loss for diffusion model
        Compute local and global structures and reverse the process to get the approximate loss
        '''
        N = atom_type.size(0)
        node2graph = batch
        ...
        target_pos_local = eq_transform(d_target[local_edge_mask],\
                                        pos_perturbed, edge_index[:, local_edge_mask],\
                                        edge_length[local_edge_mask])
        node_eq_local = eq_transform(edge_inv_local, pos_perturbed,\
    ...
    def langevin_dynamics_sample(self, atom_type, pos_init, bond_index, bond_type, batch,\
                                 num_graphs, extend_order, extend_radius=True,\
                                 n_steps=100, step_lr=0.0000010, clip=1000, clip_local=None,\
                                 clip_pos=None, min_sigma=0, is_sidechain=None,\
                                 global_start_sigma=float('inf'), w_global=0.2, w_reg=1.0, **kwargs):
        '''Compute results over timesteps for different models'''
        if self.model_type == 'diffusion':
            return self.langevin_dynamics_sample_diffusion(atom_type, pos_init, bond_index, bond_type,\
                                                           batch, num_graphs, extend_order, extend_radius, 
        ...
        return self.langevin_dynamics_sample_dsm(atom_type, pos_init, bond_index, bond_type,\
                                                    batch, num_graphs, extend_order, extend_radius, 
                                                    n_steps, step_lr, clip, clip_local, clip_pos,\
                                                    min_sigma, is_sidechain, global_start_sigma,\
                                                    w_global, w_reg)


    def langevin_dynamics_sample_diffusion(self, atom_type, pos_init, bond_index, bond_type,\
                                           batch, num_graphs, extend_order, extend_radius=True,\
                                           n_steps=100, step_lr=0.0000010, clip=1000,\
                                           clip_local=None, clip_pos=None, min_sigma=0,\
                                           is_sidechain=None,\
                                           global_start_sigma=float('inf'), w_global=0.2,\
                                           w_reg=1.0, **kwargs):
        "Compute results of diffusion model over timesteps"
        ...
        with torch.no_grad():
            ...
            for i, j in tqdm(zip(reversed(seq), reversed(seq_next)), desc='sample'):
                ...
                if sampling_type in ('generalized', 'ddpm_noisy'):
                    b = self.betas
                    t = t[0]
    ...
    def get_loss_dsm(self, atom_type, pos, bond_index, bond_type,\
        ...
        target_pos_local = eq_transform(d_target[local_edge_mask], pos_perturbed,\
                                        edge_index[:, local_edge_mask],\
                                        edge_length[local_edge_mask])
    ...
    def langevin_dynamics_sample_dsm(self, atom_type, pos_init, bond_index, bond_type, batch,\
                                     num_graphs, extend_order, extend_radius=True,\
                                     n_steps=100, step_lr=0.0000010, clip=1000, clip_local=None,\
                                     clip_pos=None, min_sigma=0, is_sidechain=None,
        ...
        with torch.no_grad():
            ...
            for i, sigma in enumerate(tqdm(sigmas, desc='sample')):
                ...
                for step in range(n_steps):
                    edge_inv_global, edge_inv_local, edge_index, edge_type, edge_length,\
...
def regularize_bond_length(edge_type, edge_length, rng=5.0):
    ...
    dist = -torch.clamp(edge_length - rng, min=0.0, max=float('inf')) * mask
    return dist
...
def clip_norm(vec, limit, order=2):
    norm = torch.norm(vec, dim=-1, p=order, keepdim=True)
